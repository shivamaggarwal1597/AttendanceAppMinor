...
tinyDB = new TinyDB(context);
    ...
    public void onBindViewHolder(@NonNull SelectBatchViewHolder holder, final int position) {
        holder.batchId.setText(batchModels.get(position).getBatch_id().toString());
        holder.currentSem.setText(batchModels.get(position).getCurrent_sem().toString());
        holder.subjectCount.setText(batchModels.get(position).getSubject_count().toString());
